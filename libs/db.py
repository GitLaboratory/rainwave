...
class SQLiteCursor(object):
    ...
    def _convert_pg_query(self, query, for_print=False):
        ...
        query = query.replace(
            "EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)", "(strftime('%s','now'))"
        )
        return query
    ...
    def execute(self, query, params=None):
        ...
        if not query:
            log.critical(
                "sqlite", "Query has not been made SQLite friendly: %s" % query
            )
    ...
    def get_next_id(self, table, column):
        ...
        if table == "r4_schedule":
            c.update(
                "INSERT INTO r4_schedule (sched_id, sid, sched_start) VALUES (%s, %s, %s)",
                (val, 0, 0),
            )
        return val
