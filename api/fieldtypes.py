...
def album_id(s, request=None):
    ...
    if (
        db.c.fetch_var(
            "SELECT COUNT(*) FROM r4_albums WHERE album_id = %s", (album_id,)
        )
        == 0
    ):
        return None
...
def artist_id(s, request=None):
    ...
    if (
        db.c.fetch_var(
            "SELECT COUNT(*) FROM r4_artists WHERE artist_id = %s", (artist_id,)
        )
        == 0
    ):
        return None
...
def sched_id(s, request=None):
    ...
    if (
        db.c.fetch_var(
            "SELECT COUNT(*) FROM r4_schedule WHERE sched_id = %s", (sched_id,)
        )
        == 0
    ):
        return None
...
def elec_id(s, request=None):
    ...
    if (
        db.c.fetch_var(
            "SELECT COUNT(*) FROM r4_elections WHERE elec_id = %s", (elec_id,)
        )
        == 0
    ):
        return None
