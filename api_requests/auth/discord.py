...
class DiscordAuth(HTMLRequest, OAuth2Mixin):
  ...
  async def get_token(self, code: str):
    ...
    try:
      async with aiohttp.ClientSession(headers=headers, loop=asyncio.get_running_loop(), timeout=DEFAULT_TIMEOUT) as session:
          async with session.post('http://discord.com/api/users/@me', data=data) as response:
  ...
  def get_user_by_discord_user_id(self, discord_user_id: str):
    return db.c.fetch_row("SELECT user_id, user_password, username FROM phpbb_users WHERE discord_user_id = %s", (discord_user_id,))
  ...
  async def register_and_login(self, token: str):
    ...
    db.c.update("INSERT INTO r4_sessions (session_id, user_id) VALUES (%(session_id), %(user_id))", (session_id, user['user_id']))
