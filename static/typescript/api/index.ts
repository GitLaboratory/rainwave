...
type Arguments<T> = [T] extends [
  (...args: infer U) => Record<string, unknown> | unknown[]
]
  ? U
  : [T] extends [void]
  ? []
...
interface RainwaveEvents {
  album: (album: unknown) => void;
  all_albums_by_cursor: (allAlbumsByCursor: unknown) => void;
  all_artists: (allArtists: unknown) => void;
  all_faves: (allFaves: unknown) => void;
  all_groups: (allGroups: unknown) => void;
  all_songs: (allSongs: unknown) => void;
  all_stations_info: (allStationsInfo: unknown) => void;
  already_voted: (alreadyVoted: unknown) => void;
  artist: (artist: unknown) => void;
  build_version: (buildVersion: unknown) => void;
  cookie_domain: (cookieDomain: unknown) => void;
  delete_request_result: (deleteRequestResult: unknown) => void;
  error_report_result: (errorReportResult: unknown) => void;
  error: (error: unknown) => void;
  fave_song_result: (faveSongResult: unknown) => void;
  group: (group: unknown) => void;
  listener: (listener: unknown) => void;
  live_voting: (liveVoting: unknown) => void;
  locale: (locale: unknown) => void;
  locales: (locales: unknown) => void;
  mobile: (mobile: unknown) => void;
  on_draw: (onDraw: unknown) => void;
  on_init: (onInit: unknown) => void;
  on_measure: (onMeasure: unknown) => void;
  pause_request_queue_result: (pauseRequestQueueResult: unknown) => void;
  playback_history: (playbackHistory: unknown) => void;
  rate_result: (rateResult: unknown) => void;
  redownload_m3u: (redownloadM3u: unknown) => void;
  relays: (relays: unknown) => void;
  request_favorited_songs_result: (requestFavoritedSongsResult: unknown) => void;
  request_line_result: (requestLineResult: unknown) => void;
  request_line: (requestLine: unknown) => void;
  request_result: (requestResult: unknown) => void;
  request_unrated_songs_result: (requestUnratedSongsResult: unknown) => void;
  requests: (requests: unknown) => void;
  sched_current: (schedCurrent: unknown) => void;
  sched_history: (schedHistory: unknown) => void;
  sched_next: (schedNext: unknown) => void;
  search_results: (searchResults: unknown) => void;
  song: (song: unknown) => void;
  station_list: (stationList: unknown) => void;
  station_song_count: (stationSongCount: unknown) => void;
  stations: (stations: unknown) => void;
  stream_filename: (streamFilename: unknown) => void;
  tip_jar: (tipJar: unknown) => void;
  top_100: (topOneHundred: unknown) => void;
  unpause_request_queue_result: (unpauseRequestQueueResult: unknown) => void;
  unrated_songs: (unratedSongs: unknown) => void;
  user_info_result: (userInfoResult: unknown) => void;
  user_recent_votes: (userRecentVotes: unknown) => void;
  user_requested_history: (userRequestedHistory: unknown) => void;
  user_search_result: (userSearchResult: unknown) => void;
  user: (user: unknown) => void;
  vote_result: (voteResult: unknown) => void;
  websocket_host: (websocketHost: unknown) => void;
}
...
export default class RainwaveApi {
  debug = false;
  ok = false;

  #listeners: {
    [K in keyof RainwaveEvents]?: Array<EventDescription<Listener<RainwaveEvents[K]>>>;
  } = {};
  ...
  constructor(
    ...
    userId: number,
    apiKey: string,
    url: string = "https://rainwave.cc/api4"
  ) {
    this.#sid = sid;
    this.#userId = userId;
    this.#apiKey = apiKey;
  ...
  private _addEvent<K extends keyof RainwaveEvents>(
    event: K,
    fn: Listener<RainwaveEvents[K]>,
    once: boolean
  ): this {
  ...
  public on<K extends keyof RainwaveEvents>(
    event: K,
    fn: Listener<RainwaveEvents[K]>
  ): this {
  ...
  public once<K extends keyof RainwaveEvents>(
    event: K,
    fn: Listener<RainwaveEvents[K]>
  ): this {
  ...
  public off<K extends keyof RainwaveEvents>(
    event: K,
    fn: Listener<RainwaveEvents[K]>
  ): this {
    if (this.#listeners[event]) {
  ...
  public removeAllListeners<K extends keyof RainwaveEvents>(event?: K): this {
    if (event) {
      delete this.#listeners[event];
    } else {
      this.#listeners = {};
  ...
  public emit<K extends keyof RainwaveEvents>(
    event: K,
    ...argv: Arguments<RainwaveEvents[K]>
  ): this {
    const listeners = this.#listeners[event];
    if (listeners) {
  ...
  //
  // Query etc.
  public listeners<K extends keyof RainwaveEvents>(
    event: K
  ): Listener<RainwaveEvents[K]>[] {
    return this.#listeners[event]?.map((description) => description.fn) || [];
  }

  public listenersCount<K extends keyof RainwaveEvents>(event: K): number {
