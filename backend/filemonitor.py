...
def _scan_all_directories(art_only=False):
    ...
    for directory, sids in config.get("song_dirs").items():
        for root, subdirs, files in os.walk(
            directory.encode("utf-8"), followlinks=True
...
def _process_album_art(filename, sids):
    ...
    try:
        ...
        for album_id in album_ids:
            im_120.save(
                "%s%s%s_%s_120.jpg"
                % (config.get("album_art_file_path"), os.sep, sids[0], album_id)
            )
            im_240.save(
                "%s%s%s_%s_240.jpg"
                % (config.get("album_art_file_path"), os.sep, sids[0], album_id)
            ...
            im_120.save(
                "%s%sa_%s_120.jpg"
                % (config.get("album_art_file_path"), os.sep, album_id)
            )
            im_240.save(
                "%s%sa_%s_240.jpg"
                % (config.get("album_art_file_path"), os.sep, album_id)
...
def _add_scan_error(filename, xception, full_exc=None):
    ...
    if (
        ...
        if full_exc:
            eo["traceback"] = traceback.format_exception(
                *full_exc
